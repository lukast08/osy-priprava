...
sem_t *semClients = nullptr;

void registerClients(int socket)
{
    sem_wait(semClients);
...
void *acceptClient(void *par)
{

    sockets *soc = (sockets *)par;
    int l_sock_client = soc->l_sock_client;
    int l_sock_listen = soc->l_sock_listen;
    // list of fd sources
...
int main(int t_narg, char **t_args)
{
    ...
    semClients = sem_open("clients", O_RDWR | O_CREAT, 0660, 1);
    ...
    while (1)
    {
        ...
        registerClients(l_sock_client);

        sockets *s = new sockets;
        s->l_sock_client = l_sock_client;
        s->l_sock_listen = l_sock_listen;
        pthread_t threadID;
        int err = pthread_create(&threadID, nullptr, acceptClient, s);
        if (err)
        {
        ...
        runningThreads.push_back(threadID);

    } // while ( 1 )

    for (pthread_t rt : runningThreads)
    {
