...
void acceptClient(int l_sock_listen, int l_sock_client)
{
    ...
    if (pid == 0)
    {
        ...
        { // communication
            ...
            // GO
            // parse request
            // GET /pic-obrazek-PP HTTP/1.0
            char *request = strtok(l_buf, " ");
            request = strtok(NULL, " ");

            char *animal;
            for (char *i = strtok(request, "-"); i != NULL; i = strtok(NULL, "-"))
            {
                animal = i;
            }
            ...
            char filename[50];
            if (strcasecmp(animal, "dog") == 0) {
                strcpy(filename, "dog.png");
            } else {
                strcpy(filename, "cat.jpg");
            }

            write(l_sock_client, responseHead, strlen(responseHead));
