...
void acceptClient(int l_sock_listen, int l_sock_client)
{
    ...
    { // communication
        ...
        if (l_read_poll[1].revents & POLLIN)
        {
            ...
            if (knowSize == 1)
            {
                printf("Reading into file\n");
                l_len = read(l_sock_client, file, sizeof(file));
            }
            else
            {
                printf("Reading into buffer\n");
                l_len = read(l_sock_client, l_buf, sizeof(l_buf));
            }
            if (!l_len)
            {
            ...
            if (knowSize == 0)
            {
                char *prefix = strtok(l_buf, ":");
                if (prefix != NULL)
                {
                    if (strcasecmp(prefix, "len") == 0)
                    {
                        char *len = strtok(NULL, ":");
                        int size = atoi(len);

                        // allocate needed space for the file;
                        file = (char *)malloc(size * sizeof(char));
                        if (file == NULL) {
                            printf("Neni volna pamet\n");
                        }
                        printf("------------------------------------------------------------file: %ld\n", sizeof(file));
                        // write data to client saying that we are ready to accept array
                        l_len = write(l_sock_client, "OK\n", sizeof("\nOK"));
                        if (l_len < 0)
            ...
            else
            {
                ...
                for (size_t i = 0; i < sizeof(file); i++)
                {
                    ...
                    finalString.append(&file[i]);
                }
                const char *finalFile = finalString.c_str();
                printf("som tu\n");
                printf("file: %s\n", file);
                // write data to client saying that we are ready to accept array
                //l_len = write(l_sock_client, finalFile, strlen(finalFile));
                //if (l_len < 0)
                //  log_msg(LOG_ERROR, "Unable to write data to stdout.");
...
int main(int t_narg, char **t_args)
{
    ...
    while (1)
    {
        ...
        acceptClient(l_sock_listen, l_sock_client);
    } // while ( 1 )
