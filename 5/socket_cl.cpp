...
slurpedFile load_file(char const *path)
{
    char *buffer = 0;
    long length;
...
int main(int t_narg, char **t_args)
{
    ...
    slurpedFile file = load_file("socket_srv.cpp");
    char fileLen[10];

    printf("len: %d\n", file.len);
    sprintf(fileLen, "len:%d", file.len);

    int l_len = write(l_sock_server, fileLen, sizeof(fileLen));
    // send data to server
    if (l_len < 0)
...
// select from fds
        ...
        if (l_read_poll[1].revents & POLLIN)
        {
            ...
            if (strcasecmp(l_buf, "OK\n") == 0) {
                gotOk = 1;
                log_msg(LOG_INFO, "Ready to send the file!");

                write(l_sock_server, file.fileBuffer, file.len);
            }
            // display on stdout
