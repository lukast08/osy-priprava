...
void acceptClient(int l_sock_client, int l_sock_listen)
{
    ...
    if (pid == 0)
    {
        ...
        { // communication
            ...
            for (char *p = strtok(l_buf, "*"); p != NULL; p = strtok(NULL, "*"))
            {
                commands.push_back(p);
            }

            char *commandsArray[commands.size() + 1];
            for (size_t i = 0; i < commands.size(); i++)
            {
                commandsArray[i] = commands[i];
            }

            char *trimmed = strtok(commandsArray[commands.size() - 1], "'$'\r\n");
            commandsArray[commands.size() - 1] = trimmed;

            commandsArray[commands.size()] = NULL;

            dup2(l_sock_client, STDOUT_FILENO);
            int err = execvp(commandsArray[0], commandsArray);
            if (err == -1)
            {
                log_msg(LOG_ERROR, "Could not exec the command!");
                exit(0);
            }
